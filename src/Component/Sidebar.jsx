...
const Sidebar = ({ mobile = false }) => {
  ...
  const { navItems, expandedItems, toggleSubmenu, isActive, isSubmenuActive, menuOpen, setMenuOpen } = useNav();
  const sidebarRef = useRef(null);

  // Handle clicks outside the sidebar to close it (only for mobile)
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (mobile && menuOpen && sidebarRef.current && !sidebarRef.current.contains(event.target)) {
        setMenuOpen(false);
      }
    };
  ...
  return (
    <aside 
      ...
      ${mobile ? `fixed inset-y-0 left-0 z-40 w-64 transform transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : '-translate-x-full'}` : 'w-64 h-screen sticky top-16'} 
